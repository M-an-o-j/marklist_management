...
def signinService(db, teacher, db_teacher):
    try:
        ...
        signin_log = TeacherSigninLogs(
            teacher_id=db_teacher.teacher_id, loggedin=datetime.now())
        access_token = create_access_token(data={"sub": str(
            db_teacher.teacher_id), "role": "teacher"}, expires_delta=expiry_del)
        db_token = TeacherToken(
            teacher_id=db_teacher.teacher_id, token=access_token)
        db.add(signin_log)
        db.add(db_token)
        db.commit()
        return JSONResponse(status_code=200, content={
            ...
            "phone_number": db_teacher.phone_number,
...
def signOutService(db, db_teacher):
    try:
        ...
        signin_user = db.query(TeacherSigninLogs).filter(
            TeacherSigninLogs.teacher_id == db_teacher.teacher_id).all()
        list_signin = [i.id for i in signin_user]
        last_login_id = max(list_signin)
        last_login = db.query(TeacherSigninLogs).filter(
            TeacherSigninLogs.id == last_login_id).first()
        last_login.loggedout = datetime.now()
        db_token = db.query(TeacherToken).filter(
            TeacherToken.teacher_id == db_teacher.teacher_id).first()
        db.delete(db_token)
        db.commit()
        return JSONResponse({
